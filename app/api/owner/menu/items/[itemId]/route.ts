...
import { UpdateMenuItemSchema } from '@/schemas/owner/menu-item'
import { updateMenuItem } from '@/services/owner/menu-item.service'
...
export async function PATCH(
  ...
  try {
    const result = await updateMenuItem(sessionResult.data.restaurantId, itemId, parsed.data)
    if (!result.success) {
    ...
    return NextResponse.json({ success: false, code: 'INTERNAL_ERROR', message: 'Failed to update menu item.' }, { status: 500 })
  }
}
