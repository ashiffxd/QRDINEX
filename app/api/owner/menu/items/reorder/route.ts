...
import { ReorderMenuItemsSchema } from '@/schemas/owner/menu-item'
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    const result = await reorderMenuItems(sessionResult.data.restaurantId, parsed.data)
    if (!result.success) {
    ...
    return NextResponse.json({ success: false, code: 'INTERNAL_ERROR', message: 'Failed to reorder menu items.' }, { status: 500 })
  }
}
