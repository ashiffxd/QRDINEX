...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!result.success || !result.session) {
      // Map domain errors to safe HTTP messages
    ...
    // If it's a newly created session (or even resumed), ensure the cookie is set
    // It's safe to overwrite the cookie with the same token on resume.
    ...
    cookieStore.set({
      ...
      secure,
      sameSite: 'lax',
      maxAge: 60 * 60 * 12, // 12 hours (sessions shouldn't last longer than a day)
    })
    ...
    if (result.isNew) {
      try {
        ...
        if (table) {
          socketEmitter.emitToRestaurant(result.session.restaurantId, SESSION_EVENTS.NEW, {
